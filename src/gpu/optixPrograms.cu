...
bool shouldDistort()
{
	return ctx.rayTimeOffsets != nullptr && ctx.sensorAngularVelocity != nullptr & ctx.sensorLinearVelocity != nullptr;
}
...
extern "C" __global__ void __raygen__()
{
	...
	if (shouldDistort()) {
		...
		ray = Mat3x4f::TRS(ctx.rayTimeOffsets[rayIdx] * (*ctx.sensorLinearVelocity * 0.001f),
		                   ctx.rayTimeOffsets[rayIdx] * (*ctx.sensorAngularVelocity * 0.001f * toDeg))
		      * ray;
	}
